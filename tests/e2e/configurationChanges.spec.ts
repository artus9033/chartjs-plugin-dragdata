...
describeEachChartType(function testGenerator(fileName, scenario) {
	...
	test.beforeEach("Setup page for test", async ({ page, isMobile }) => {
	...
	for (const enablerLocationSpec of ALL_ENABLER_LOCATION_SPECS.filter(
		...
		test(`data dragging behaviour changes appropriately upon ${enablerLocationSpec} dragData configuration property changes`, async ({
			...
			async function updateChartDragDataEnabledConfig(bEnabled: boolean) {
				await test.step(`Update dragdata config with '${bEnabled}' for axis: ${enablerLocationSpec}`, async () => {
					await page.evaluate(
			...
			await test.step(`test dragging when disabled in config for axis: ${enablerLocationSpec}`, async () => {
			...
			await test.step(`test dragging when enabled in config for axis: ${enablerLocationSpec}`, async () => {
