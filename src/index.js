...
function updateData (chartInstance, callback) {
	return () => {
		if (element && event) {
			...
			const value = e.touches === undefined ? chartInstance.scales[scale].getValueForPixel(e.clientY-chartInstance.canvas.getBoundingClientRect().top) : chartInstance.scales[scale].getValueForPixel(e.touches[0].clientY-chartInstance.canvas.getBoundingClientRect().top)
