...
let element: InteractionItem,
  ...
  floatingBar: boolean,
  initValue: number,
  curDatasetIndex: number,
  curIndex: number,
  eventSettings: AnimationSpec<keyof ChartTypeRegistry>;
let isDragging = false;

function getSafe<TType extends keyof ChartTypeRegistry>(
  func: () => AnimationSpec<TType>
) {
  try {
    return func();
  } catch (e) {
...
const getElement = (
  e: any,
  chartInstance: Chart,
  callback: Options<any, false>["onDrag"]
) => {
  ...
  if (element) {
    ...
    if (type === "bar") {
      ...
      initValue = newPos - (curValue as number);
    }
...
const dragEndCallback = (e: any, chartInstance: Chart, callback) => {
  curIndex = undefined;
  isDragging = false;
...
export type Options<Data, Booleanish = true> = Booleanish extends true
  ? false | _Options<Data>
  : _Options<Data>;
...
const ChartJSdragDataPlugin = {
  id: "dragdata",
  afterInit: function (chartInstance: Chart) {
    if (
      chartInstance.config.options.plugins &&
      chartInstance.config.options.plugins.dragData
    ) {
      const pluginOptions = chartInstance.config.options.plugins.dragData;
      select<Element, unknown>(chartInstance.canvas).call(
        drag()
  ...
  beforeEvent: function (chart: Chart) {
    if (isDragging) {
      (chart.tooltip as any)?.update?.();
      return false;
    }
  },
};
